...
class Pellets (object):
    ...
    def check (self, pellets_normal, pellets_special, pacman):
        for i, p in enumerate (pellets_normal [:]):
            p_rect = pygame.draw.rect(Surface, (255, 255, 255), p)
            if pacman.colliderect(p_rect):
                del pellets_normal [i]
                pacman.score += 10

        for i, p in enumerate (pellets_special [:]):
            p_rect = (Surface, (255, 255, 255), p)
            if pacman.colliderect(p_rect):
                del pellets_special [i]
                pacman.score += 50